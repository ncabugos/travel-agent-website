...
## Plan
- [x] Migration: re-apply 019 `handle_new_user` (email + role) and backfill `email` from auth.users
      for rows where it is null. Apply to prod via MCP so history stays in sync.
- [x] Callback: `/api/agent-portal/auth-callback` accepts `token_hash` + `type` and calls
      `verifyOtp`. Keeps `code` branch for Google OAuth. Works in any browser.
- [x] Templates in `docs/supabase-email-templates/`: confirmation, magic-link, recovery. Style guide
...
- [x] `scripts/push-auth-email-templates.mjs`: pushes subjects + templates via Management API.
- [x] Supabase auth config: custom SMTP via Resend (smtp.resend.com:465, sender = EMAIL_FROM).
- [x] Committed b174d3f on `auth/branded-email-magic-link`, fast-forwarded main, pushed. Vercel building.
- [x] Verified on production: token generated by curl and opened in a separate browser landed on
      /agent-portal/onboarding; a real emailed link (read back from Gmail) created a session.
- [ ] Operator: run the wizard end to end as nick@wineandwellnesstravel.com and confirm the admin
...
## Review (2026-09-08)
- Shipped b174d3f: callback verifies `token_hash` with verifyOtp; migration re-applies the 019 trigger
  and backfills email (nick@wineandwellnesstravel.com, josie@relaxury.travel); three branded templates
  pushed; SMTP via Resend.
- Sender ended up as `no-reply@mail.eliteadvisorhub.com` with a scoped Resend key
  (id 45b9775a-f2ae-474f-88a7-41ea661d9a84), not nick@eliteadvisorhub.com. Cause: the Gmail connector
  double-decodes quoted-printable, so delivered links looked corrupted (`token_hash-…`). RAW MIME and
  Resend's stored copy were correct the whole time. Kept the subdomain because auth links now go out
