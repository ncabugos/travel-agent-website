...
export async function generateStaticParams() {
  const slugs = await getPrivateJourneySlugs('jet')
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata({
    ...
    description:
      journey.tagline ??
      journey.description ??
      `Private jet journey planning with ${agent.agency_name}.`,
    path: `private-jets/${journeySlug}`,
  })
}

export default async function PrivateJetDetailPage({ params }: PageProps) {
  const { agentId, journeySlug } = await params
  const journey = await getPrivateJourney(journeySlug)
  if (!journey || journey.journey_type !== 'jet') notFound()

  return (
    <T2JourneyDetail
      journey={journey}
      agentId={agentId}
      indexHref="/private-jets"
      indexLabel="All private jet journeys"
    />
  )
}
