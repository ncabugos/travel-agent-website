...
async function fetchAgents(): Promise<AdminAgentRow[]> {
  const res = await fetch('/api/admin/agents')
  const data = await res.json()
  return (Array.isArray(data) ? data : []) as AdminAgentRow[]
}

const emptyInvite = { full_name: '', agency_name: '', email: '', tier: 'starter', template: 'frontend' }

/**
 * Agents table + invite modal. The initial rows arrive from the server page
 * (no spinner on first paint); after an invite the list is refreshed from the
 * API so the new row appears without a reload.
 */
export function AgentsTable({ initialAgents }: { initialAgents: AdminAgentRow[] }) {
