# AGENTS.md — Elite Advisor Hub (EAH)

> This file is auto-loaded by Codex at session start. It is the project's **constitution** —
...
memory/               Long-lived conversational learning (separate from AGENTS.md, see §11)
...
5. **URL composition goes through `lib/tenant-paths.ts`.** Don't build `/frontend/${agentId}/...`
   strings inline — past refactors got bitten by this (see `.Codex/settings.local.json` for the
...
## 11. AGENTS.md vs. `memory/` — read this once

- **AGENTS.md** (this file + nested ones in subdirectories) = **stable project constitution**.
  Always loaded. Edit only when the architecture, conventions, or strategy changes.
- **`memory/`** = **evolving learning across sessions**. Things you (Codex) discovered or were
...
Rule of thumb: if you'd say "this is true about EAH forever" → AGENTS.md. If you'd say "Nick
...
Subdirectory AGENTS.md files extend (not override) this one and are loaded when work happens in
their tree. Current nested files:

- `app/AGENTS.md` — App Router conventions, route ownership, RSC vs client component rules
- `components/AGENTS.md` — Template isolation, design-token discipline, naming
- `supabase/AGENTS.md` — Migration discipline, RLS patterns, common queries
- `lib/AGENTS.md` — Which Supabase client to import where; data-fetching boundaries
- `app/agent-portal/AGENTS.md` — Onboarding wizard contract, Tiptap config, Stripe portal handoff
- `app/admin/AGENTS.md` — Super-admin RLS expectations, notification routing
...
`karpathy-guidelines` skill (`.Codex/skills/karpathy-guidelines/SKILL.md`).
